...
if(isset($_GET["id"]) && !empty($_GET["id"])){
    ...
    $updateReservation = mysqli_query($connection, "UPDATE `reservations` SET status = 'Completed' WHERE id = '" . $reservation_id . "'");
    
    if($updateReservation){
        
        $updateTransaction = mysqli_query($connection, "UPDATE `transactions` SET check_out_date = '". date('Y-m-d') ."' WHERE reservation_id = '" . $reservation_id . "'");
        ...
        if($updateTransaction){
            ...
            if(mysqli_num_rows($getReservationDetails) != 0) {
                ...
                if($updateRoom){
                    echo "<script>";
                    echo "alert('Reservation successfully checked-out.');";
